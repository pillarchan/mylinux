...
class bt_api:
    ...
    def create_site_single(self,domain,rewrite_url):
        now = datetime.datetime.now()
        success_sites = {}
        str_result = {}
        ...
        if (domain != ''):
            ...
            if add_result.get('siteStatus') != None:
                if (add_result['siteStatus'] == True):
                    ...
                    ### 修改 跳转 URL
                    URL = str(rewrite_url).strip()
                    file = c_file()
                    data = file.set_index_html('/opt/script/jenkins_domain/bt/index.html',URL)
    ...
    def replace_index_html(self,domain,rewrite_url):
    ...
    def only_create_site(self,domain,rewrite_url):
        now = datetime.datetime.now()
        success_sites = {}
        str_result = {}
...
if __name__ == '__main__':
    script,domain,rewrite_url,action = sys.argv

    str_domain = str(domain).strip()
    str_url = str(rewrite_url).strip()
    str_action = str(action).strip()

    bt_tz = bt_api()

    if str_action == 'create_site':
        bt_tz.create_site_single(str_domain,str_url)
    elif str_action == 'only_create_site':
        bt_tz.only_create_site(str_domain,str_url)
    elif  str_action == 'search':
        bt_tz.search(str_domain)
    elif  str_action == 'apply_cert':
        bt_tz.only_apply_cert(str_domain,str_url)
    elif  str_action == 'replace_URL':
        bt_tz.replace_index_html(str_domain,str_url)
    else:
        print('操作不存在')
