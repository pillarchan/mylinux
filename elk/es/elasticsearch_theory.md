...
```
	...
	流程如下，下面是罗列的主分片和副本分片上成功新建，删除，更新一个文档必要的顺序步骤:
	
	客户端请求->coordinating节点->根据id到主节点分片进行写操作并保存数据->转发请求到副本分片同步数据->副本分片节点保存数据成功后告知主分片节点->主节点接受副节点响应后，反馈执行结果到客户端
...
流程如下，罗列在主分片或者复制分片上检索一个文档必要的顺序步骤:
客户端请求->coordinating节点->根据id到节点对应的分片，如果主分片繁忙，会将请求转发给副本分片节点->分片节点取得查询结果后将结果先返回给协调节点->协调节点再返回给客户端
...
流程如下：
客户端请求->coordinating节点->协调节点创建一个优化级的空队列->转发请求到各分片主或副分片->各分片本地执行查询，结果储存到有序本地优先级队列->每个分片返回文档(document)的"_id"和它优先级队列里的所有document的排序值给协调节点，协调节点把这些值合并到自己的优先队列里产生全局排序结果->协调节点辨别出哪个文档(document)的数据需要取回，并且向相关分片发出GET请求->每个分片加载document并且根据查询条件和_id值，然后再将document的数据返回协调节点->document的数据都被取回，协调节点会将结果返回给客户端。
       搜索(query)阶段包含以下三步:
...
流程如下：
客户端更新请求->协调节点->协调节点通过计算，将请求发送主分片->主分片根据请求_id和_source修改数据，保存数据后->将修改后完整文档的新版本转发给副本分片->副本分片同步成功后，返回成功给主分片->主分片将返回的信息发给协调节点->协调节点再将成功信息返回客户端
 部分更新一个文档的步骤如下:
...
客户端请求->coordinating节点->根据id到节点对应的分片并行请求数据->分片节点取得查询结果后将结果先返回给协调节点->协调节点再返回给客户端	
...
事务日志流程：
客户端写操作请求->通过协调节点->请求到主分片->根据_id进行写操作->被修改后的数据先保存在内存区的segment，在此生成事务日志文件保存至对应索引目录下的translog文件->刷新数据到文件系统缓存->再次刷新落盘到对就的磁盘区域，此时translog.ckp会检验日志文件，并将旧的日志文件删除
```
