...
MYSQL_HOME_DIR="${MYSQL_SOFTWARE_DIR}/mysql57"
...
MYSQL_PACKAGE="$(ls /usr/local/src/ | grep mysql-5.7)"
MYSQL_NAME=$(echo $MYSQL_PACKAGE | awk -F'.tar' '{print $1}')


mkdir -pv /opt/{data,logs,software,etc} $MYSQL_DATA_DIR $MYSQL_ETC_DIR $MYSQL_LOGS_DIR/{tmp,undo,redo,slow,relay,genaral,binlog}
...
$MYSQL_BIN/mysqld --defaults-file=$MYSQL_ETC_DIR/my.cnf --user=$MYUSER --basedir=$MYSQL_HOME_DIR --datadir=$MYSQL_DATA_DIR --initialize-insecure
